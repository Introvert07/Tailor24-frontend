...
export default function ProductDetailPage() {
  ...
  const place = async () => {
    if (!token) {
      toast.error('Authentication required for commissions');
  ...
  return (
    <div style={{ background:C.page, minHeight:'100vh', paddingTop: '90px' }}>
      ...
      <div style={{ background:C.maroon, borderBottom:`1px solid ${C.maroonXL}`, position:'fixed', top:0, left:0, right:0, zIndex:100 }}>
          ...
          <div style={{ background:C.white, border:`1px solid ${C.borderL}`, boxShadow:`0 4px 20px ${C.ink}06` }}>
            ...
            <div style={{ padding:'clamp(20px,3vw,32px) clamp(20px,3vw,36px)' }}>
              <AnimatePresence mode="wait">
                ...
                {step === 3 && (
                  <motion.div key="s3" initial={{ opacity:0, scale:.98 }} animate={{ opacity:1, scale:1 }}>
                    ...
                    <motion.button onClick={place} disabled={placing}
                      style={{ width:'100%', marginTop:24, padding:'17px', background:C.maroon, border:'none', cursor:'pointer', color:'white', fontWeight:700, letterSpacing:'0.4em', display:'flex', alignItems:'center', justifyContent:'center', gap:12 }}>
              ...
              {step < 3 && (
                <div style={{ display:'flex', gap:10, marginTop:32, paddingTop:24, borderTop:`1px solid ${C.borderXL}` }}>
                  {step > 0 && (
                    <button onClick={() => setStep(s => s - 1)} style={{ padding:'13px 22px', border:`1.5px solid ${C.borderL}`, background:'none', cursor:'pointer', color:C.muted, display:'flex', alignItems:'center', gap:8 }}>
                      <FiArrowLeft size={10}/> Back
                    </button>
                  )}
                  <button onClick={next} style={{ flex: 1, padding: '13px 22px', background: C.maroon, border: 'none', cursor: 'pointer', color: C.white, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
