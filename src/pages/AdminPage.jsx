    ...
    <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
      ...
      <motion.div initial={{scale:.93,y:16}} animate={{scale:1,y:0}}
        ...
        <div style={{background:danger?C.maroon:C.parchment,padding:'20px 24px 16px'}}>
          ...
          <p style={{fontSize:12,color:danger?'rgba(255,255,255,0.6)':C.muted,lineHeight:1.7,margin:0}}>{msg}</p>
        </div>
...
function AddressCell({order}) {
  ...
  if (!addr && !order.city) return <span style={{fontSize:10,color:C.muted,fontStyle:'italic'}}>—</span>;
  return (
    <div style={{fontSize:11,color:C.ink,lineHeight:1.6,minWidth:140}}>
      {addr?.street && <div style={{fontWeight:600,color:C.ink}}>{addr.street}</div>}
      {addr?.landmark && <div style={{fontSize:9.5,color:C.muted}}>Near: {addr.landmark}</div>}
      <div style={{fontSize:9.5,color:C.gold,fontWeight:700,letterSpacing:'0.12em',textTransform:'uppercase',marginTop:2}}>
        {order.city}{addr?.pincode ? ` – ${addr.pincode}` : ''}
      </div>
      {addr?.state && addr.state !== 'Madhya Pradesh' && (
        <div style={{fontSize:8.5,color:C.muted}}>{addr.state}</div>
      )}
    </div>
  );
}
...
function OrderExpandRow({order,colSpan}) {
  ...
  return (
    <tr>
      <td colSpan={colSpan} style={{padding:0,borderBottom:`2px solid ${C.border}`}}>
        <motion.div
          initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}}
          exit={{opacity:0,height:0}} transition={{duration:.22}}
          style={{background:`${C.gold}08`,borderTop:`1px dashed ${C.borderL}`,
            padding:'14px 20px',display:'flex',flexWrap:'wrap',gap:24}}>

          {/* Phone from populated userId */}
          {order.userId?.phone && (
            <div style={{minWidth:140}}>
              <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
                <FiPhone size={11} color={C.gold}/>
                <Lbl t="Contact" c={C.gold}/>
              </div>
          ...
          <div style={{minWidth:200}}>
            <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
              <FiMapPin size={11} color={C.gold}/>
              <Lbl t="Delivery Address" c={C.gold}/>
            </div>
            {addr ? (
              <div style={{fontSize:12,color:C.ink,lineHeight:1.8}}>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:600,color:C.ink}}>
                  {addr.street}
                </div>
                {addr.landmark && <div style={{fontSize:11,color:C.muted}}>Near: {addr.landmark}</div>}
                <div style={{fontSize:11,color:C.ink}}>
                  {order.city}{addr.pincode ? ` – ${addr.pincode}` : ''}
                </div>
                <div style={{fontSize:10,color:C.muted}}>{addr.state || 'Madhya Pradesh'}</div>
              </div>
            ) : (
              <span style={{fontSize:12,color:C.muted,fontStyle:'italic'}}>No address saved</span>
            )}
          </div>
          ...
          <div style={{minWidth:160}}>
            <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
              <FiPackage size={11} color={C.gold}/>
              <Lbl t="Order Details" c={C.gold}/>
            </div>
            <div style={{fontSize:11,color:C.ink,lineHeight:1.8}}>
              <div><span style={{color:C.muted}}>Garment: </span>{order.garmentType||'—'}</div>
              <div><span style={{color:C.muted}}>Fit Method: </span>
                <span style={{textTransform:'capitalize'}}>
                  {order.measurements?.method?.toLowerCase().replace('_',' ')||'—'}
                </span>
              </div>
              <div><span style={{color:C.muted}}>Fabric: </span>{order.fabric?.source||'—'}</div>
            </div>
          </div>
          ...
          {order.consultantNotes && (
            <div style={{minWidth:180,flex:1}}>
              <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
                <Lbl t="Consultant Notes" c={C.gold}/>
              </div>
              <p style={{fontSize:11,color:C.ink,lineHeight:1.7,margin:0,
                fontStyle:'italic',borderLeft:`2px solid ${C.gold}`,paddingLeft:10}}>
                {order.consultantNotes}
              </p>
            </div>
          )}
...
export default function AdminPage() {
  ...
  const fOrders = orders.filter(o=>{
    ...
    return (!q||o._id?.slice(-6).toLowerCase().includes(q)||
      ...
      o.address?.pincode?.toLowerCase().includes(q)||
      o.city?.toLowerCase().includes(q)||
      o.userId?.phone?.includes(q))&&   // ← phone from populated user
  ...
  return (
    <div style={{minHeight:'100vh',background:C.page,fontFamily:"'Montserrat',sans-serif",color:C.ink,paddingTop:64}}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        .atr{cursor:pointer;transition:background .15s;}
        .atr:hover{background:${C.parchment}!important;}

        .admin-stat-grid{display:grid;grid-template-columns:repeat(8,1fr);gap:10px;margin-bottom:24px;}
        @media(max-width:1100px){.admin-stat-grid{grid-template-columns:repeat(4,1fr);}}
        @media(max-width:640px) {.admin-stat-grid{grid-template-columns:repeat(2,1fr);gap:8px;}}

        .admin-tabs{display:flex;border-bottom:2px solid ${C.borderL};margin-bottom:18px;flex-wrap:wrap;overflow-x:auto;-webkit-overflow-scrolling:touch;}
        .admin-tab-btn{display:flex;align-items:center;gap:6px;padding:10px 16px;background:transparent;border:none;cursor:pointer;border-bottom:2px solid transparent;margin-bottom:-2px;font-family:'Montserrat',sans-serif;font-size:8.5px;letter-spacing:0.3em;font-weight:700;text-transform:uppercase;transition:color .2s;white-space:nowrap;flex-shrink:0;}
        @media(max-width:480px){.admin-tab-btn{padding:10px 10px;font-size:7.5px;gap:4px;}}

        .admin-table-wrap{background:${C.white};border:1px solid ${C.borderL};width:100%;overflow-x:auto;-webkit-overflow-scrolling:touch;}
        .admin-table-wrap table{width:100%;min-width:700px;border-collapse:collapse;}

        .admin-filters{display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;align-items:center;}
        .admin-filters select{padding:9px 12px;background:${C.white};border:1px solid ${C.border};font-family:'Montserrat',sans-serif;font-size:8.5px;color:${C.ink};cursor:pointer;outline:none;letter-spacing:0.15em;text-transform:uppercase;max-width:100%;}
        @media(max-width:480px){.admin-filters{gap:6px;}.admin-filters select{font-size:8px;padding:8px 10px;}}

        .admin-header-inner{max-width:1280px;margin:0 auto;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;position:relative;z-index:2;padding:0 4px;}
        .admin-content{max-width:1280px;margin:0 auto;padding:20px 24px;}
        @media(max-width:768px){.admin-content{padding:16px 14px;}}
        @media(max-width:480px){.admin-content{padding:12px 10px;}}

        .consult-summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:18px;}
        @media(max-width:640px){.consult-summary{grid-template-columns:repeat(2,1fr);gap:8px;}}

        .consult-fields{padding:16px 20px;display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:0 28px;}
        @media(max-width:480px){.consult-fields{grid-template-columns:1fr;padding:12px 14px;}}

        @media(max-width:640px){.col-city{display:none;}.col-addr{display:none;}}

        .inq-filter-btn{padding:8px 12px;border-width:1px;border-style:solid;cursor:pointer;font-family:'Montserrat',sans-serif;font-size:8px;letter-spacing:0.3em;text-transform:uppercase;font-weight:700;white-space:nowrap;flex-shrink:0;}
        @media(max-width:480px){.inq-filter-btn{padding:7px 8px;font-size:7px;letter-spacing:0.2em;}}

        .action-area{display:flex;gap:6px;flex-wrap:wrap;}

        .admin-title{font-family:'Cormorant Garamond',serif;font-size:clamp(18px,3vw,32px);color:white;font-weight:700;margin:0;line-height:1;}
        .admin-subtitle{font-size:7px;letter-spacing:0.5em;color:rgba(240,192,64,0.7);text-transform:uppercase;margin-bottom:3px;}
        @media(max-width:380px){.admin-subtitle{display:none;}}
      `}</style>
        ...
        <div className="admin-header-inner">
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            <div>
              ...
              <h1 className="admin-title">Admin <em style={{color:C.goldL}}>Dashboard</em></h1>
            </div>
          </div>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
            <div style={{textAlign:'right'}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,color:'white',fontWeight:600}}>{user?.name}</div>
              <div style={{fontSize:7,letterSpacing:'0.4em',color:`${C.goldL}70`,textTransform:'uppercase',marginTop:1}}>{user?.role}</div>
            </div>
            <button onClick={load} style={{padding:9,background:'rgba(255,255,255,0.08)',
              border:`1px solid ${C.goldL}35`,cursor:'pointer',display:'flex',alignItems:'center',flexShrink:0}}>
      ...
      <div className="admin-content">
        ...
        <div className="admin-stat-grid">
          {STAT_CARDS.map((s,i)=>(
            <motion.div key={i} initial={{opacity:0,y:14}} animate={{opacity:1,y:0}}
              ...
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:28,color:s.c,fontWeight:700,lineHeight:1}}>{s.v}</div>
              <div style={{fontSize:7,letterSpacing:'0.22em',color:C.muted,textTransform:'uppercase',marginTop:4,lineHeight:1.4}}>{s.l}</div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="admin-tabs">
          {TABS.map(t=>(
            <button key={t.id} className="admin-tab-btn"
              onClick={()=>{setTab(t.id);setSearch('');setStatusF('All');setExpandedOrder(null);}}
              style={{borderBottomColor:tab===t.id?C.maroon:'transparent',color:tab===t.id?C.maroon:C.muted}}>
              <t.icon size={12}/>
              {t.label}
        ...
        <AnimatePresence mode="wait">
          ...
          {/* ══ ORDERS ══ */}
          {tab==='orders' && (
            <motion.div key="orders" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div className="admin-filters">
                <SearchBar val={search} onChange={setSearch} ph="Search ID, patron, address, phone…"/>
                <select value={statusF} onChange={e=>setStatusF(e.target.value)}>
                  <option value="All">All Statuses</option>
                  {ALL_STATUSES.map(s=><option key={s} value={s}>{STATUS_LABEL[s]}</option>)}
                </select>
                <div style={{padding:'9px 12px',background:C.parchment,border:`1px solid ${C.borderL}`,
                  fontSize:8.5,color:C.muted,letterSpacing:'0.2em',textTransform:'uppercase',whiteSpace:'nowrap',flexShrink:0}}>
              ...
              <div className="admin-table-wrap">
                <table>
                  <TH cols={['Order ID','Patron','Delivery Address','Status','Actions']}/>
                  <tbody>
                    {fOrders.length===0 ? <EmptyRow cols={5} label="No orders found"/>
                    : fOrders.map((o,i)=>{
                      ...
                      return (
                        <>
                          <tr key={o._id} className="atr"
                            ...
                            style={{borderBottom: isExpanded ? 'none' : `1px solid ${C.borderL}`,
                              background: isExpanded ? `${C.gold}0A` : i%2===0 ? C.white : C.page,
                              transition:'background .15s'}}>

                            <td style={{padding:'12px 14px'}}>
                              <div style={{display:'flex',alignItems:'center',gap:6}}>
                                <div>
                                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,color:C.maroon,fontWeight:700}}>
                                ...
                                <FiChevronDown size={11} color={C.muted}
                                  style={{marginLeft:4,transition:'transform .2s',
                                    transform:isExpanded?'rotate(180deg)':'none',flexShrink:0}}/>
                              </div>
                            </td>

                            <td style={{padding:'12px 14px'}}>
                              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,color:C.ink,fontWeight:600}}>
                                {o.userId?.name||'—'}
                              </div>
                              ...
                              {o.userId?.phone && (
                                <div style={{display:'flex',alignItems:'center',gap:4,marginTop:3}}>
                                  <FiPhone size={8} color={C.gold}/>
                                  <span style={{fontSize:9,color:C.gold,letterSpacing:'0.05em'}}>{o.userId.phone}</span>
                                </div>
                              )}
                            </td>

                            <td className="col-addr" style={{padding:'12px 14px'}}>
                              <AddressCell order={o}/>
                            </td>
                                ...
                                {!TERMINAL.includes(o.status)&&(
                                  <button onClick={()=>{setModal({type:'cancelOrder',id:o._id});setMInput('');}}
                                    style={{display:'flex',alignItems:'center',gap:4,padding:'5px 10px',
                                      ...
                                      fontFamily:"'Montserrat',sans-serif",fontSize:7.5,
                                      letterSpacing:'0.2em',color:C.red,fontWeight:700,textTransform:'uppercase',whiteSpace:'nowrap'}}>
                                    <FiX size={9}/>Cancel
                                  </button>
                                )}
                              </div>
                          ...
                          <AnimatePresence>
                            {isExpanded && (
                              <OrderExpandRow key={`expand-${o._id}`} order={o} colSpan={5}/>
                            )}
                          </AnimatePresence>
                        </>
                      );
              ...
              <div style={{marginTop:10,fontSize:8,color:C.muted,letterSpacing:'0.2em',
                textTransform:'uppercase',textAlign:'right'}}>
                Click any row to expand full address, phone & order details
              </div>
            </motion.div>
          )}
          ...
          {/* ══ USERS ══ */}
          {tab==='users' && (
            <motion.div key="users" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div className="admin-filters">
                    ...
                    : fUsers.map((u,i)=>{
                      ...
                      return (
                        <tr key={u._id} className="atr"
                          style={{borderBottom:`1px solid ${C.borderL}`,background:i%2===0?C.white:C.page,transition:'background .15s'}}>
                          <td style={{padding:'12px 14px'}}>
                            <div style={{display:'flex',alignItems:'center',gap:10}}>
                              ...
                              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,color:C.ink,fontWeight:600}}>{u.name}</span>
                            </div>
                          </td>
                          <td style={{padding:'12px 14px'}}>
                          ...
                          <td className="col-city" style={{padding:'12px 14px',fontSize:9,color:C.gold,letterSpacing:'0.2em',textTransform:'uppercase'}}>{u.city||'—'}</td>
                          <td style={{padding:'12px 14px'}}>
                            <span style={{padding:'3px 8px',fontSize:7.5,fontWeight:700,letterSpacing:'0.2em',textTransform:'uppercase',
                              background:u.role==='admin'?`${C.maroon}15`:u.role==='staff'?`${C.teal}15`:`${C.gold}15`,
                              color:u.role==='admin'?C.maroon:u.role==='staff'?C.teal:C.muted,whiteSpace:'nowrap'}}>
                              {u.role}
                            </span>
                          </td>
                          <td style={{padding:'12px 14px',fontFamily:"'Cormorant Garamond',serif",fontSize:20,color:C.maroon,fontWeight:700}}>{oCount}</td>
                          <td style={{padding:'12px 14px'}}>
          ...
          {/* ══ INQUIRIES ══ */}
          {tab==='inquiries' && (
            <motion.div key="inq" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div className="admin-filters">
                <SearchBar val={inqSearch} onChange={setInqSearch} ph="Search name, phone, subject…"/>
                {['All','new','read','replied'].map(f=>(
                  <button key={f} onClick={()=>setInqF(f)} className="inq-filter-btn"
                    style={{borderColor:inqF===f?C.maroon:C.borderL,background:inqF===f?C.maroon:'transparent',color:inqF===f?'white':C.muted}}>{f}</button>
                ))}
              </div>
              <div className="admin-table-wrap">
                    ...
                    : fInquiries.map((inq,i)=>{
                      ...
                      return (
                        <tr key={inq._id} className="atr"
                          style={{borderBottom:`1px solid ${C.borderL}`,background:inq.status==='new'?`${C.goldB}05`:i%2===0?C.white:C.page}}>
                          <td style={{padding:'12px 14px',whiteSpace:'nowrap'}}>
                            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:700,color:C.ink}}>{inq.name}</div>
                            {inq.status==='new'&&<span style={{fontSize:7,color:C.goldB,letterSpacing:'0.3em',fontWeight:700}}>● NEW</span>}
                          </td>
                          ...
                          <td style={{padding:'12px 14px',maxWidth:150}}>
                            <span style={{fontSize:9,color:C.maroon,fontWeight:700,letterSpacing:'0.1em',textTransform:'uppercase'}}>{inq.subject}</span>
                          </td>
                          <td style={{padding:'12px 14px',maxWidth:200}}>
                            <p style={{margin:0,fontSize:11,color:C.ink,lineHeight:1.6,
                              display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{inq.message}</p>
                          </td>
                          <td style={{padding:'12px 14px'}}>
                            <span style={{display:'inline-flex',alignItems:'center',padding:'3px 9px',
                              background:`${sc}15`,fontSize:7.5,letterSpacing:'0.2em',color:sc,fontWeight:700,textTransform:'uppercase'}}>{inq.status}</span>
                          </td>
                          ...
                          <td style={{padding:'12px 14px'}}>
                            <div className="action-area">
                              {inq.status==='new'&&<button onClick={()=>updateInqStatus(inq._id,'read')} style={actionBtn(C.teal)}>Read</button>}
                              {inq.status!=='replied'&&<button onClick={()=>updateInqStatus(inq._id,'replied')} style={actionBtn(C.green)}>Replied</button>}
                              <button onClick={()=>setModal({type:'deleteInquiry',id:inq._id,name:inq.name})} style={actionBtn(C.red)}><FiTrash2 size={9}/></button>
                            </div>
                          </td>
                        </tr>
                      );
          ...
          {/* ══ CONSULTATIONS ══ */}
          {tab==='consultations' && (
            <motion.div key="cons" initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:.3}}>
              <div className="consult-summary">
                {Object.entries(CONSULT_S).map(([k,m])=>(
                  <div key={k} style={{background:C.white,border:`1px solid ${C.borderL}`,borderTop:`3px solid ${m.c}`,padding:'12px 16px',display:'flex',alignItems:'center',gap:10}}>
                    <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:26,color:m.c,fontWeight:700,lineHeight:1}}>
              ...
              <div className="admin-filters">
                ...
                {['All','Pending','Confirmed','Completed'].map(f=>(
                  <button key={f} onClick={()=>setCF(f)} className="inq-filter-btn"
                    style={{borderColor:cF===f?C.maroon:C.borderL,background:cF===f?C.maroon:'transparent',color:cF===f?'white':C.muted}}>{f}</button>
                ))}
              </div>
                  ...
                  {fConsults.map((c,i)=>{
                    ...
                    return (
                      <motion.div key={c._id} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} transition={{delay:i*.04}}
                        style={{background:C.white,border:`1px solid ${C.borderL}`,borderLeft:`3px solid ${sm.c}`,overflow:'hidden'}}>
                        <div style={{padding:'14px 20px',background:C.parchment,borderBottom:`1px solid ${C.borderL}`,
                          display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
                          <div style={{display:'flex',alignItems:'center',gap:12}}>
                            <div style={{width:36,height:36,borderRadius:'50%',background:`${C.maroon}10`,border:`1px solid ${C.border}`,
                              display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,
                              fontFamily:"'Cormorant Garamond',serif",fontSize:17,color:C.maroon,fontWeight:700}}>
                              {c.name?.charAt(0)?.toUpperCase()||'?'}
                            </div>
                            <div>
                              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:19,color:C.ink,fontWeight:700,lineHeight:1}}>{c.name||'—'}</div>
                              <div style={{fontSize:9,color:C.muted,marginTop:3,display:'flex',alignItems:'center',gap:5,flexWrap:'wrap'}}>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                            <span style={{display:'inline-flex',alignItems:'center',padding:'3px 9px',background:`${sm.c}15`,fontSize:7.5,letterSpacing:'0.2em',color:sm.c,fontWeight:700,textTransform:'uppercase'}}>{sm.l}</span>
                            {Object.entries(CONSULT_S).filter(([k])=>k!==c.status).map(([k,m])=>(
                              <button key={k} onClick={()=>updateConsultStatus(c._id,k)}
                                style={{padding:'3px 9px',border:`1px solid ${m.c}40`,background:`${m.c}08`,cursor:'pointer',
                                  fontFamily:"'Montserrat',sans-serif",fontSize:7,letterSpacing:'0.25em',textTransform:'uppercase',fontWeight:700,color:m.c,whiteSpace:'nowrap'}}>
                                {m.l}
                              </button>
                            ))}
                            <button onClick={()=>setModal({type:'deleteConsult',id:c._id,name:c.name||c.mobileNumber})}
                              style={{padding:'5px 8px',background:`${C.red}0D`,border:`1px solid ${C.red}35`,cursor:'pointer',display:'flex',alignItems:'center',color:C.red,flexShrink:0}}>
                              <FiTrash2 size={12}/>
                            </button>
                          </div>
                        </div>
                        ...
                        <div className="consult-fields">
                          {[
                            {icon:FiUsers, label:'Full Name',     val:c.name},
                            {icon:FiPhone, label:'Mobile Number', val:c.mobileNumber},
                            {icon:FiClock, label:'Time Slot',     val:c.timeSlot},
                          ].map(({icon:Icon,label,val})=>(
                            <div key={label} style={{padding:'10px 0',borderBottom:`1px solid ${C.borderL}`,display:'flex',gap:10,alignItems:'flex-start'}}>
                              <Icon size={13} color={C.gold} style={{marginTop:3,flexShrink:0}}/>
                              <div>
                                <div style={{marginBottom:3}}><Lbl t={label}/></div>
                                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,color:C.ink,fontWeight:600,letterSpacing:'0.04em'}}>
                                  {val||'—'}
                                </div>
                              </div>
                            </div>
                          ))}
                          <div style={{gridColumn:'1/-1',padding:'10px 0',borderBottom:`1px solid ${C.borderL}`,display:'flex',gap:10,alignItems:'flex-start'}}>
                            <FiMapPin size={13} color={C.gold} style={{marginTop:3,flexShrink:0}}/>
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{marginBottom:3}}><Lbl t="Home Address"/></div>
                              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:16,color:C.ink,fontWeight:600,lineHeight:1.6,wordBreak:'break-word'}}>
                                {c.address||'—'}
                              </div>
                            </div>
                          </div>
